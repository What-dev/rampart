...
pub fn encrypt(pw: String, master_password: Vec<u8>) -> Vec<u8> {
...
pub fn decrypt(encrypted_pw: Vec<u8>, master_password: Vec<u8>) -> String {
	let key = derive_key_from_password(master_password);
	let iv = hex!("000102030405060708090a0b0c0d0e0f");
	let cipher = Aes128Cbc::new_from_slices(&key, &iv).unwrap();
...
fn derive_key_from_password(master_password: Vec<u8>) -> [u8; 16] {
	let mut key = [0; 16];
	for (i, byte) in master_password.iter().enumerate() {
		key[i % 16] ^= byte;
	}
	key
}
